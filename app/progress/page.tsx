...
export default function ProgressPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const statsRes = await fetch(
          `/api/progress/stats?startDate=${startDate}&endDate=${endDate}`
        )
        if (statsRes.ok) {
          ...
          if (statsData.ok) {
            ...
            console.error('Stats API returned ok=false or missing ok field')
          }
        } else {
          const errorData = await statsRes.json().catch(() => ({ error: 'Unknown error' }))
          console.error('Stats API request failed:', statsRes.status, errorData)
        }
        ...
        // Load team stats (if supervisor or admin)
        if (session?.user?.role === 'supervisor' || session?.user?.role === 'admin') {
          const teamRes = await fetch(
            `/api/progress/team?startDate=${startDate}&endDate=${endDate}`
          )
          if (teamRes.ok) {
    ...
    loadData()
  }, [period, customStart, customEnd, session?.user?.role])
